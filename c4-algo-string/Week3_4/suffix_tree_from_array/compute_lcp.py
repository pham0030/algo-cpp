...
def compute_lcp_array(S, order):
    len_S = len(S)
    lcp_array = [None] * (len_S-1)
    lcp = 0
    pos_in_order = invert_suffix_array(order)
    suffix = order[0]
    for i in range(len_S):
        order_index = pos_in_order[suffix]
        if order_index == len_S - 1:  # last suffix element in A
            lcp = 0  # cant find next element in A, so reset
            suffix = (suffix + 1) % len_S  # next suffix in string
            continue
        next_suffix = order[order_index + 1]  # next suffix in suffix array
...
if __name__ == '__main__':
    ...
    print(order)
    lcp_array = compute_lcp_array(text, order)
    print(lcp_array)
